...
import hashlib
import uuid
from Beautiful_DatabaseObject import *
...
def main_screen_admin():
    ...
    Button(main_win, text='Log Out', bg=bgc, fg=fgc, command=lambda : \
           (main_win.destroy(), Login())).place(x=x_cord * 1 / 2,
            y=y_cord * 9 / 10, anchor='center')
...
def complete_quiz(win):
    ...
    quiz.configure(bg=bgc)
    db.retrieve_quizes()
    print(db.quiz_details)
    print((db.quiz_details)[0])
    for quiz in range(len(db.quiz_details)):
        for detail in range(len(db.quiz_details[quiz])):
            Label(quiz, text=db.quiz_details[detail],bg=bgc,fg=fgc,font=def_font).place(x=x_cord*(quiz+1)/5,y=y_cord*detail/6,anchor='center')
