...
class Database:
    ...
    def __init__(self):
        ...
        while not self.success:
            # Used a try statement so that if there is a connection error then the program does not crash immediately
            try:
    ...
    def search_user(self, target):
        self.conn.execute("SELECT USERNAME FROM USERS WHERE USERNAME='" + target + "'")
    ...
    def search_password(self, password, user):
        ...
        self.open_data()
        self.conn.execute(
            "SELECT ENCRYPT_PASS FROM USERS WHERE USERNAME='" + user + "'"
        )
        results = self.conn.fetchone()
    ...
    def create_account(self, user, password, conf_password, admin, email, target):
        if admin == "True":
            admin = 1
            if target != "N/A":
    ...
    def add_question(self, details):
        ...
        if timer.isdigit():
            ...
            self.conn.execute(
                "SELECT QUESTION_ID FROM QUESTIONS WHERE QUESTION_TEXT='" + q_text + "'"
            )
...
# Method to remove a question from the tree widget and current questions
    ...
    def retrieve_quizes(self, complete=0):
        self.conn.execute(
            "SELECT USER_ID FROM USERS WHERE USERNAME='" + self.current_user + "'"
        )
        ...
        for quiz in range(len(quizzes)):
            self.conn.execute(
                "SELECT * FROM QUIZ WHERE QUIZ_ID='" + str(quizzes[quiz][0]) + "'"
            )
    ...
    def start_test(self, test):
        ...
        for details in range(len(self.question_id)):
            ...
            self.question_details.append(self.conn.fetchall())
    ...
    def end_test(self, score, question):
        ...
        sql = (
            "UPDATE ASSIGNED_QUIZ SET COMPLETE = %s, SCORE = %s, DATE_COMPLETED = %s WHERE USER = %s AND "
            "QUIZ_ASSIGNED = %s "
        )
        vals = (
            1,
    ...
    def retrieve_completed(self, query_type, user=0):
        ...
        if query_type == 1:
            self.conn.execute("SELECT USER_ID FROM USERS WHERE USERNAME='" + user + "'")
            user_id = str(self.conn.fetchone()[0])
            self.conn.execute(
                "SELECT QUIZ_ASSIGNED FROM ASSIGNED_QUIZ WHERE USER='"
                + user_id
            ...
            for quiz_id in range(len(quizzes_completed)):
                self.conn.execute(
                    "SELECT DATE_SET FROM QUIZ WHERE QUIZ_ID='"
                    + str(quizzes_completed[quiz_id][0])
                    + "'"
                )
                dates_set.append(self.conn.fetchone())
            self.personal_progress = []
            self.conn.execute(
                "SELECT SCORE FROM ASSIGNED_QUIZ WHERE USER='"
                + user_id
                + "'AND COMPLETE = 1"
            )
            for row in self.conn:
    ...
    def user_target(self, num, user=0):
        if user == 0:
            user = self.current_user
        self.conn.execute("SELECT TARGET FROM USERS WHERE USERNAME='" + user + "'")
    ...
    def user_weakness(self):
        ...
        for i in range(len(quiz_scores)):
            self.conn.execute(
                "SELECT TOPIC FROM QUIZ WHERE QUIZ_ID ='" + str(quiz_scores[i][0]) + "'"
            )
